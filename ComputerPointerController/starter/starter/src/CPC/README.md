...
1. `fd` : Specify path to xml file of face detection model
2. `fl` : Specify path to xml file of landmark regression model
3. `hp` : Specify path to xml file of Head Pose Estimation model
4. `gz` : Specify path to xml file of Gaze Estimation model
5. `i` : Specify path to input Video file or cam for Webcam
6. `flag`: flag from ffd, ffl, fhp, fgz represent face detection, landmark regression, head pose estimation, gaze estimation models respectively. Help with popping up a window for the preview.

Other optional arguments include
7. `probs`: probability threshold, default=0.6
8. `d`: Specify Device for inference, the device can be CPU, GPU, FPGU, MYRID
9. `o` : Specify path to output folder where we will store results
